...
const MovingImage = ({ title, img, link }) => {
  ...
  function handleCursorIn(event) {
    console.log({ event });
    imageRef.current.style.display = "inline-block";
    x.set(event.pageX - 40);
    y.set(-240);
  }
...
const FeaturedArticle = ({ img, title, time, summary, link }) => {
  return (
    <li
      ...
      "
    >
      <div className="absolute top-0 -right-4 -z-10 w-[102%] h-[103%] rounded-[2.5rem] bg-dark rounded-br-3xl dark:bg-light" />
      <Link
        href={link}
        target="_blank"
